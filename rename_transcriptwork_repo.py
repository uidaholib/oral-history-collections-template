#!/usr/bin/env python3
"""
Rename transcript CSV files in transcripts-from-transcriptwork-repo to match objectid from lcoh.csv.
...
Usage:
    python3 rename_transcriptwork_repo.py          # Dry run (shows changes)
    python3 rename_transcriptwork_repo.py --execute # Actually rename files
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Rename transcript files in transcripts-from-transcriptwork-repo to match objectids from lcoh.csv'
    )
    parser.add_argument(
        '--execute',
        action='store_true',
    ...
    transcripts_dir = script_dir / "_data" / "transcripts-from-transcriptwork-repo"
    ...
    if not transcripts_dir.exists():
        print(f"Error: Could not find {transcripts_dir}")
        return
    ...
    print(f"\nFound {len(rename_map)} potential objectid mappings")
    ...
    renamed, skipped, errors = rename_files_in_directory(
    ...
    if not args.execute:
        ...
        print(f"\n✓ Files renamed in {transcripts_dir}")
        print(f"\nNext steps:")
        print(f"  1. Review the renamed files in {transcripts_dir}")
        print(f"  2. Copy them to _data/transcripts/ to restore overwritten files")
        print(f"     Example: cp {transcripts_dir}/*.csv _data/transcripts/")
